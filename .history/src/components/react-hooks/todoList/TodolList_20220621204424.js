...
import { addTaskApi, deleteTaskById, getTaskApi } from "../../../apis/index";
...
const TodolList = () => {
  ...
  const handleActionTask = async (itemId, action) => {
    const listTasksClone = [...datas];
    if (action === "check") {
      const index = listTasksClone.findIndex((item) => item.id === itemId);
      const states = listTasksClone[index].state;
      const newTask = {
